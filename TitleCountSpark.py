...
counts = words.filter(lambda word: word.lower() not in stopWords).map(lambda word: (word, 1)).reduceByKey(lambda a, b: a + b)
#swap key and val so that key is count and value is word. Then sort in descending order first
sortedCounts = counts.map(lambda w: (w[1], w[0])).sortByKey(ascending = False)
